...
import { Area, AreaChart, Bar, BarChart, Brush, CartesianGrid, Line, LineChart, ReferenceLine, ResponsiveContainer, Tooltip, XAxis, YAxis } from "recharts";
import { z } from "zod";
...
function Replay() {
  ...
  return <Section id="replay" index="04" title="REPLAY" className="replay-section"><Reveal><h2 className="section-title">Drag the past back.</h2><p className="section-copy">Pick a moment. VOID replays compensations in LIFO order across every connected system and shows you what came back and what did not.</p></Reveal><Reveal><div className="ascii-box crop crop-extra replay-shell" data-testid="replay-demo"><div className="timeline-head"><span className="kicker">LIVE LEDGER / 14 ACTIONS</span><span className="mono-small">09:02 → {actions[position].time}</span></div><div className="timeline"><div className="timeline-line" /><motion.div className="timeline-fill" animate={{ width: `${(position / (actions.length - 1)) * 100}%` }} transition={{ type: "spring", stiffness: 160, damping: 22 }} /><div className="timeline-nodes">{actions.map((item, index) => <motion.button type="button" className={`timeline-node ${index <= position ? "active" : ""}`} key={`${item.label}-${index}`} onClick={() => setPosition(index)} aria-label={`Jump to ${item.time}, ${item.label}`} whileHover={{ y: -3 }} whileTap={{ scale: .92 }}><i title={item.tone} style={{ background: index <= position ? `var(--${item.tone === "r0" ? "ok" : item.tone === "r1" ? "accent-hex" : item.tone === "r2" ? "warn" : "bad"})` : "var(--paper)" }} /><span>{item.time}</span></motion.button>)}</div></div><input data-testid="input-replay-scrubber" className="replay-range" type="range" min="0" max={actions.length - 1} value={position} onChange={(event) => setPosition(Number(event.target.value))} aria-label="Replay timeline" /><AnimatePresence mode="wait" initial={false}><motion.div className="replay-focus" key={actions[position].label} initial={{ opacity: 0, x: 12 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -12 }} transition={{ duration: .2 }}><span className="kicker">{actions[position].system} / {actions[position].tone.toUpperCase()}</span><strong>{actions[position].label}</strong><span>preflight {position < 6 ? "prepared" : "sealed"} · inverse {actions[position].tone === "r3" ? "none, budget required" : "ready"}</span></motion.div></AnimatePresence><div className="replay-panels">{rows.map(([system, value]) => <motion.div layout className="system-panel" key={system}><h4>{system}</h4><div><span>state</span><span>{value}</span></div><div><span>comp</span><span>{position < 6 && system === "LEDGER" ? "none" : "ready"}</span></div><div><span>hash</span><span>0x9f3c..</span></div></motion.div>)}</div>{count.irreversible > 0 && <div className="replay-note">cannot be reversed, R3, logged and budgeted</div>}<div className="replay-counter">reversed {count.reversed} · mitigated {count.mitigated} · irreversible {count.irreversible}</div><div className="caption">fig. 03, compensation replay</div></div></Reveal></Section>;
}

function Topology() {
  const [mode, setMode] = useState("MCP proxy");
  const shapes: Record<string, { left: string[]; right: string[]; note: string }> = {
    "MCP proxy": { left: ["LangGraph", "CrewAI", "custom loop"], right: ["MCP tools", "payments", "data + mail"], note: "one endpoint, every tool surface" },
    "SDK wrap": { left: ["Python agent", "TypeScript agent", "worker queue"], right: ["wrapped tools", "snapshots", "signed ledger"], note: "native in the agent process" },
    Sidecar: { left: ["agent runtime", "job runner", "batch worker"], right: ["HTTP tools", "databases", "cloud APIs"], note: "isolated beside the runtime" },
  };
  const shape = shapes[mode];
  return <Section id="topology" index="05" title="TOPOLOGY"><Reveal><h2 className="section-title">One layer, in the write path.</h2></Reveal><Reveal><div className="ascii-box topology"><div className="beams" aria-hidden="true">───────&gt;───────&gt;───────&gt;───────</div><div className="topology-grid"><AnimatePresence mode="wait" initial={false}><motion.div className="node-stack" key={`${mode}-left`} initial={{ opacity: 0, x: -12 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 12 }} transition={{ duration: .25 }}>{shape.left.map((node) => <motion.div layout className="node" key={node} whileHover={{ y: -3, borderColor: "var(--accent-hex)", color: "var(--accent-hex)" }}>{node}</motion.div>)}</motion.div></AnimatePresence><motion.div className="node void-node" animate={{ boxShadow: ["0 0 0 color-mix(in srgb, var(--accent-hex) 0%, transparent)", "0 0 28px color-mix(in srgb, var(--accent-hex) 18%, transparent)", "0 0 0 color-mix(in srgb, var(--accent-hex) 0%, transparent)"] }} transition={{ duration: 2.8, repeat: Infinity }}>[ VOID ]<small className="mono-small">preflight / inverse / ledger</small></motion.div><AnimatePresence mode="wait" initial={false}><motion.div className="node-stack" key={`${mode}-right`} initial={{ opacity: 0, x: 12 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -12 }} transition={{ duration: .25 }}>{shape.right.map((node) => <motion.div layout className="node" key={node} whileHover={{ y: -3, borderColor: "var(--accent-hex)", color: "var(--accent-hex)" }}>{node}</motion.div>)}</motion.div></AnimatePresence></div><div className="topology-note">{shape.note}</div><div className="caption">fig. 04, deployment topology</div></div></Reveal><div className="deployment-chips">{Object.keys(shapes).map((item) => <button data-testid={`button-mode-${item.replace(" ", "-").toLowerCase()}`} key={item} onClick={() => setMode(item)} className={`chip-toggle ${mode === item ? "selected" : ""}`}>{item}</button>)}<span className="mono-small">active shape: {mode}</span></div></Section>;
}

function Classes() {
  const [open, setOpen] = useState<Tone | null>("r0");
  return <Section id="classes" index="06" title="CLASSES"><Reveal><h2 className="section-title">Know the class before the call.</h2></Reveal><div className="class-list">{classRows.map((row) => <div className="class-row" key={row.id}><button data-testid={`button-class-${row.id}`} className="class-trigger" onClick={() => setOpen(open === row.id ? null : row.id)} aria-expanded={open === row.id}><i className="class-chip" style={{ background: `var(--${row.id === "r0" ? "ok" : row.id === "r1" ? "accent-hex" : row.id === "r2" ? "warn" : "bad"})` }} /><strong>{row.id.toUpperCase()}</strong><span>{row.title}, {row.subtitle}</span><b>{open === row.id ? "−" : "+"}</b></button><AnimatePresence initial={false}>{open === row.id && <motion.div className="class-detail" initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} transition={{ duration: .25 }}><div>{row.body}</div><div><span className="mono-small">COMPENSATION</span><br />{row.example}</div></motion.div>}</AnimatePresence></div>)}</div></Section>;
}
...
function Telemetry() {
  ...
  const latency = [
    { week: "01", value: 41 }, { week: "02", value: 37 }, { week: "03", value: 34 },
    { week: "04", value: 35 }, { week: "05", value: 29 }, { week: "06", value: 27 },
    { week: "07", value: 25 }, { week: "08", value: 24 }, { week: "09", value: 22 },
    { week: "10", value: 20 }, { week: "11", value: 18 }, { week: "12", value: 17 },
  ];
  const protectedActions = [
    { week: "01", value: 12 }, { week: "02", value: 18 }, { week: "03", value: 24 },
    { week: "04", value: 31 }, { week: "05", value: 39 }, { week: "06", value: 46 },
    { week: "07", value: 54 }, { week: "08", value: 63 }, { week: "09", value: 71 },
    { week: "10", value: 79 }, { week: "11", value: 88 }, { week: "12", value: 96 },
  ];
  const radius = [
  ...
  const riskMix = [
    { tool: "R0", value: 63 }, { tool: "R1", value: 23 }, { tool: "R2", value: 10 }, { tool: "R3", value: 4 },
  ];
  const [metric, setMetric] = useState<"coverage" | "latency" | "actions">("coverage");
  const [windowSize, setWindowSize] = useState(12);
  const [radiusMode, setRadiusMode] = useState<"systems" | "classes">("systems");
  const [budget, setBudget] = useState(37);
  const lineSets = {
    coverage: { data: coverage, title: "reversible coverage", suffix: "%", description: "coverage rises as inverses compile", color: "var(--accent-hex)", threshold: 80 },
    latency: { data: latency, title: "preflight latency", suffix: " ms", description: "preflight gets lighter as plans cache", color: "var(--ok)", threshold: 40 },
    actions: { data: protectedActions, title: "protected actions", suffix: "k", description: "protected write volume by week", color: "var(--warn)", threshold: 75 },
  } as const;
  const activeLine = lineSets[metric];
  const lineData = activeLine.data.slice(-windowSize);
  const barData = radiusMode === "systems" ? radius : riskMix;
  ...
  return <Section id="telemetry" index="07" title="TELEMETRY"><Reveal><h2 className="section-title">The numbers your risk team will ask for.</h2></Reveal><div className="chart-grid">
    <Reveal className="ascii-box crop chart-box wide bklit-chart"><div className="chart-heading"><div><div className="chart-title">{activeLine.title} / {windowSize} weeks</div><strong className="chart-readout">{lineData[lineData.length - 1]?.value}{activeLine.suffix}</strong></div><div className="chart-controls" role="tablist" aria-label="Telemetry metric"><button className={metric === "coverage" ? "active" : ""} onClick={() => setMetric("coverage")} aria-pressed={metric === "coverage"}>coverage</button><button className={metric === "latency" ? "active" : ""} onClick={() => setMetric("latency")} aria-pressed={metric === "latency"}>latency</button><button className={metric === "actions" ? "active" : ""} onClick={() => setMetric("actions")} aria-pressed={metric === "actions"}>volume</button></div></div><motion.div className="recharts-wrap" key={metric} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: .3 }}><ResponsiveContainer width="100%" height={210}><AreaChart data={lineData} margin={{ top: 12, right: 8, left: -25, bottom: 18 }}><defs><linearGradient id="telemetryFill" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor={activeLine.color} stopOpacity={.3} /><stop offset="100%" stopColor={activeLine.color} stopOpacity={0} /></linearGradient></defs><CartesianGrid stroke="var(--line)" strokeDasharray="2 4" /><XAxis dataKey="week" tick={{ fill: "var(--ink-faint)", fontSize: 9, fontFamily: "var(--app-font-mono)" }} tickLine={false} axisLine={{ stroke: "var(--line-strong)" }} /><YAxis domain={metric === "latency" ? [0, 50] : [0, 100]} tick={{ fill: "var(--ink-faint)", fontSize: 9, fontFamily: "var(--app-font-mono)" }} tickLine={false} axisLine={false} /><ReferenceLine y={activeLine.threshold} stroke="var(--warn)" strokeDasharray="3 5" label={{ value: "watch", fill: "var(--warn)", fontSize: 9 }} /><Tooltip contentStyle={tooltipStyle} cursor={{ stroke: activeLine.color, strokeDasharray: "2 3" }} formatter={(value) => [`${value}${activeLine.suffix}`, activeLine.title]} /><Area type="monotone" dataKey="value" stroke={activeLine.color} fill="url(#telemetryFill)" strokeWidth={2} dot={{ r: 2, fill: activeLine.color, strokeWidth: 0 }} activeDot={{ r: 5, fill: activeLine.color, stroke: "var(--paper)", strokeWidth: 2 }} /><Brush dataKey="week" height={18} stroke={activeLine.color} travellerWidth={8} /></AreaChart></ResponsiveContainer></motion.div><div className="chart-footer"><span className="mono-small">{activeLine.description}</span><label className="range-label">window <input type="range" min="6" max="12" value={windowSize} onChange={(event) => setWindowSize(Number(event.target.value))} /></label></div><div className="caption">fig. 05, interactive telemetry</div></Reveal>
    <Reveal className="ascii-box chart-box stagger-1 bklit-chart"><div className="chart-heading"><div><div className="chart-title">{radiusMode === "systems" ? "blast radius / system" : "action mix / class"}</div><strong className="chart-readout">{barData.reduce((total, item) => total + item.value, 0)}<small> index</small></strong></div><div className="chart-controls" role="tablist" aria-label="Blast radius view"><button className={radiusMode === "systems" ? "active" : ""} onClick={() => setRadiusMode("systems")} aria-pressed={radiusMode === "systems"}>systems</button><button className={radiusMode === "classes" ? "active" : ""} onClick={() => setRadiusMode("classes")} aria-pressed={radiusMode === "classes"}>classes</button></div></div><motion.div className="recharts-wrap" key={radiusMode} initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: .25 }}><ResponsiveContainer width="100%" height={210}><BarChart data={barData} margin={{ top: 12, right: 4, left: -25, bottom: 0 }}><CartesianGrid stroke="var(--line)" strokeDasharray="2 4" vertical={false} /><XAxis dataKey="tool" tick={{ fill: "var(--ink-faint)", fontSize: 9, fontFamily: "var(--app-font-mono)" }} tickLine={false} axisLine={{ stroke: "var(--line-strong)" }} /><YAxis tick={{ fill: "var(--ink-faint)", fontSize: 9, fontFamily: "var(--app-font-mono)" }} tickLine={false} axisLine={false} /><Tooltip contentStyle={tooltipStyle} cursor={{ fill: "var(--accent-hex)", opacity: 0.08 }} formatter={(value) => [`${value}`, "risk index"]} /><Bar dataKey="value" fill="var(--ink)" activeBar={{ fill: "var(--accent-hex)" }} radius={[2, 2, 0, 0]} /></BarChart></ResponsiveContainer></motion.div><div className="mono-small">hover a bar for the risk index</div><div className="caption">fig. 06, interactive risk surface</div></Reveal>
    <Reveal className="ascii-box chart-box stagger-2 bklit-chart"><div className="chart-heading"><div><div className="chart-title">budget burned / today</div><strong className="chart-readout">{budget}%<small> used</small></strong></div><span className="live-pill"><i /> live</span></div><motion.div className="ring enhanced-ring" style={{ background: `conic-gradient(var(--accent-hex) ${budget * 3.6}deg, var(--line-strong) ${budget * 3.6}deg)` }} initial={{ rotate: -24, scale: 0.8, opacity: 0 }} whileInView={{ rotate: 0, scale: 1, opacity: 1 }} viewport={{ once: true }} transition={{ duration: 0.9, ease: [0.16, 1, 0.3, 1] }}><span>{budget}%</span></motion.div><label className="budget-control">simulate R3 spend <input type="range" min="0" max="100" value={budget} onChange={(event) => setBudget(Number(event.target.value))} /></label><div className="mono-small" style={{ textAlign: "center", marginTop: 10 }}>billing-ops / {Math.ceil(budget / 20)} of 5 R3</div><div className="caption">fig. 07, live budget simulator</div></Reveal>
...
function Faq() {
  ...
  return <Section id="faq" index="12" title="FAQ"><Reveal><h2 className="section-title">The uncomfortable questions, answered plainly.</h2></Reveal><div className="faq-list">{faqs.map(([question, answer], index) => <div className="faq-row" key={question}><button data-testid={`button-faq-${index}`} className="faq-trigger" onClick={() => setOpen(open === index ? -1 : index)} aria-expanded={open === index}><span>{question}</span><b>{open === index ? "−" : "+"}</b></button><AnimatePresence initial={false}>{open === index && <motion.div className="faq-answer" initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} transition={{ duration: .25 }}>{answer}</motion.div>}</AnimatePresence></div>)}</div></Section>;
}
