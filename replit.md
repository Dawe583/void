...
- `packages/`: the product itself, six packages under Apache 2.0. `@void/{registry,ledger,proxy,policy,connectors,cli}`. Being built one work package at a time per `docs/EXECUTION-PLAN.md`; `docs/DECISIONS.md` records why each foundational choice was made and what was rejected.
- `api/_registry.ts`: the Reversibility Registry. 89 calls, 178 preconditioned cases, 35 vendors. Read by three consumers: the Vercel Function, the Express route and the browser bundle for `/registry`, all through the `@shared` alias.
...
- **A reversibility class is a function of target state, not of the call.** `api/_registry.ts` therefore stores an ordered list of preconditioned cases per entry rather than one class, and the first precondition that holds decides. 74 of the 89 entries change class with configuration, which is the whole argument for the registry existing: a policy engine that classifies by call name is wrong most of the time. Every entry ends with an unguarded `always` case so there is always a verdict.
- **The registry is one module, not a copy per consumer.** `artifacts/void/vite.config.ts` aliases `@shared` to `api/`, so the page, the endpoint and the classification that would run at intercept time cannot disagree. That is also why `_registry.ts` imports nothing: a workspace package would break the Vercel Function, and `node:*` would break the browser bundle. Request parsing lives in `_registry_http.ts` so the bundle does not carry it.
...
- **The palette is warm paper and burnt orange.** Light: paper `#f6f0e3`, ink `#17150f`, one accent `#bd4206`. Dark: ground `#12100a`, ink `#f0eade`, accent `#e8702c`. The four reversibility classes keep their own ramp (green, blue, ochre, crimson) so they never read as the accent, and every one of those values clears 4.5:1 on its ground, because the labels are 10px mono. Deepening the paper costs accent contrast, so the two move together: `#c44608` fell to 4.37:1 on `#f6f0e3` and had to go a step darker.
- **In the dark theme, presence comes from the rules, not from the accent.** It used to read flat and glaring at once, and those were one fault: the rules sat at 1.20:1 against the ground, so the spec sheet structure was invisible, which left the accent at 7.57:1 as the only bright thing on the page. Rules are now 1.47:1 and 2.20:1 and the accent is 6.14:1. If the dark theme ever looks dull again, raise the rules before touching the accent.
- **The visual language is a Swiss print spec sheet crossed with a terminal:** paper ground, hairline rules, crosshairs at every rule end, crop marks on every figure, figure numbers, and monospace annotation. Serif is reserved for headlines, the sans carries body copy, everything else is mono. No shadows, no gradients, no glass, no aurora. Cells are separated by rules rather than by gaps and cards. If a change would add a shadow or a rounded card, it is fighting the system.
...
- **An SVG overlay drawn on hardcoded coordinates over an HTML layout will not touch the boxes it is meant to connect.** The deployment topology used `viewBox="0 0 1000 320"` with `preserveAspectRatio="none"` and coordinates like `M300 60 C 400 60...` behind a CSS grid, so the wires floated in the middle at every width. The fix is to measure: walk `offsetParent` from each node up to the stage, size the SVG to the stage in real pixels, and build the paths from those numbers. Measure with `offsetLeft`/`offsetTop` rather than `getBoundingClientRect`, because rects include CSS transforms and the nodes carry an entrance animation. Re-measure on a `ResizeObserver` and after `document.fonts.ready`, since web fonts change every box height after first paint.
- **`api/tsconfig.json` pins `rootDir` and the typecheck asserts the emit LAYOUT, not just the exit code.** Without the pin, one relative import out of `api/` raises TypeScript's inferred emit root to the repository root, and every compiled function moves from `<out>/registry.js` to `<out>/api/registry.js` at once. Vercel then resolves no route at all, and `tsc` exits 0 while it happens. This is why `packages/registry` re-exports from `api/_registry.ts` rather than the file moving into the package.
- **`node --test` exits 0 when it finds no test files.** A package whose tests were deleted or renamed out of the glob reports green. `pnpm test` therefore goes through `scripts/src/check-tests.mjs`, which fails any package reporting zero tests. Both that guard and the failing test guard are proven by temporarily breaking them, not asserted.
- **Never let motion drive a transform on an SVG element that carries a `transform` attribute.** A CSS transform overrides the presentation attribute, so `<motion.g transform="translate(x y)" animate={{ y }} />` renders every node at the origin, stacked. All eight taint graph nodes collapsed into one before this was caught. Keep the layout translate on a plain `<g>` and animate an inner `<motion.g>`, which has no attribute of its own to lose.
- **The terminal classes are `.terminal-bar` and `.terminal-body`,** not `term-*`. A misspelled class silently renders unstyled rather than failing, which is how an unspaced, unpadded transcript panel shipped through a typecheck and a build.
