...
- **The palette is warm paper and burnt orange.** Light: paper `#faf7f0`, ink `#17150f`, one accent `#c44608`. Dark is the same system on a warm near black with `#ff7a33`. The four reversibility classes keep their own ramp (green, blue, ochre, crimson) so they never read as the accent, and every one of those values clears 4.5:1 on the paper because the labels are 10px mono.
